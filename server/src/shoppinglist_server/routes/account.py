...
def register_routes(bp):
    ...
    def change_password_view():
        ...
        accounts.change_password(
            ...
            g.shoppinglist_token,
        )
        audit.record("account.password_changed", account_id=g.shoppinglist_account.id)
        return "", 204
    ...
    def change_email_view():
        ...
        accounts.change_email(
            conn, g.shoppinglist_account.id, data.get("password"), data.get("new_email")
        )
        # The new address itself is deliberately not logged — see audit.py on keeping PII out.
        audit.record("account.email_changed", account_id=g.shoppinglist_account.id)
        return "", 204
    ...
    def list_sessions_view():
        conn = get_db()
        sessions = accounts.list_sessions(conn, g.shoppinglist_account.id, g.shoppinglist_token)
    ...
    def revoke_session_view(session_id):
        conn = get_db()
        accounts.revoke_session(conn, g.shoppinglist_account.id, session_id)
        # The auth_tokens row id, never the token itself — it names the session without being
        # usable as a credential (T-121).
        audit.record(
            "auth.session_revoked", account_id=g.shoppinglist_account.id, session_id=session_id
        )
        return "", 204
    ...
    def delete_account_view():
        data = json_body()
        conn = get_db()
        accounts.delete_account(conn, g.shoppinglist_account.id, data.get("password"))
        audit.record("account.deleted", account_id=g.shoppinglist_account.id)
        return "", 204
    ...
    def get_settings_view():
        conn = get_db()
        return jsonify(accounts.get_settings(conn, g.shoppinglist_account.id)), 200
    ...
    def update_settings_view():
        ...
        result = accounts.update_settings(conn, g.shoppinglist_account.id, **kwargs)
